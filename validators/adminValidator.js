...
export const moderateCourseSchema = Joi.object({
  ...
  moderationNote: Joi.string().messages({
...
export const submitReportSchema = Joi.object({
  contentType: Joi.string().valid('course', 'lesson', 'quiz', 'user')
    .required().messages({
...
export const reviewReportSchema = Joi.object({
  status: Joi.string().valid('reviewed', 'resolved', 'dismissed')
    .required().messages({
      'any.only': 'Status must be one of: reviewed, resolved, dismissed',
      'any.required': 'Status is required'
    }),
  reviewNote: Joi.string().required().messages({
